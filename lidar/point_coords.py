...
# ----------------------------------------------------------------------------
# https://github.com/isl-org/Open3D/blob/main/examples/python/visualization/mouse_and_point_coord.py
...
class ExampleApp:
    ...
    def __init__(self, cloud):
        ...
        self.window = app.create_window("Open3D - GetCoord Example", 1280, 720)
        ...
        mat.base_color = np.array([0.37,0.31,0.3,1])
        ...
        self.widget3d.scene.add_geometry("Terrain", cloud, mat)

        bounds = self.widget3d.scene.bounding_box
        center = bounds.get_center()
        self.widget3d.setup_camera(60, bounds, center)
        self.widget3d.look_at(center, center - [0, 1, 0], [0, 0, 1])
        ...
        if event.type == gui.MouseEvent.Type.BUTTON_DOWN and event.is_modifier_down(gui.KeyModifier.CTRL):
            ...
            def depth_callback(depth_image):
                ...
                if depth == 1.0:  # clicked on nothing (i.e. the far plane)
                    ...
                    ray *= depth/n
                    #ray = np.array([a,b,1])
                    # Rinv = np.array([[ 0.29292164,  0.11824451, -0.94879669],
                    #                 [ 0.95612768, -0.04047481,  0.29014073],
                    #                 [-0.00409482, -0.99215927, -0.12491279]])
                    Ext_inv = np.array([[ 2.92921636e-01,  1.18244508e-01, -9.48796686e-01, 7.96641606e+05],
                        [ 9.56127684e-01, -4.04748081e-02,  2.90140728e-01, 6.38364048e+06],
                        [-4.09481608e-03, -9.92159275e-01, -1.24912795e-01, 7.91643588e+02],
                        [ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00, 1.00000000e+00]])
                    
                    # t = np.array([-6.33692571e+06,  1.64963565e+05, -1.09620430e+06])
                    # X,Y,Z = Rinv.dot(np.add([a,b,depth],-t))
                    X,Y,Z, _ = Ext_inv.dot(np.append(ray,[1]))
                    print(f'{depth=}')
                    print(f'{X=}, {Y=},{Z=},')
...
def main():
    ...
    #mesh = o3d.io.read_triangle_mesh(datadir+'mesh.ply')
